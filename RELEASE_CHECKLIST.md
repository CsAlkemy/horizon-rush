...
- [ ] Cover images — REOPENED (2026-08-13). The generated set was replaced with
      `branding/noxrush.png`, a 1280x720 illustrated banner. It is the better
      marketing image and is now the README header, but it cannot be submitted as
      a cover: it carries five feature captions (one of them literally "PLAY
      NOW", their own example of a banned caption), it is under the required
      1920x1080, and two of the three mandatory ratios are not 16:9. Either
      re-export the art at >=1920x1080 with no captions and a 2:3-safe
      composition, or re-run `node scripts/make-branding.mjs` for a compliant set
- [ ] Fix the banner's "MULTIPLAYER — PLAY WITH FRIENDS" claim before using it
      anywhere public: the portal build has FRIENDS removed (no server), so that
      line is false for the version being submitted
